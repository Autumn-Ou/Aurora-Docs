A report from the device containing data from the device's [Settings Registry](./registry). This can be sent as a response to a [[Write Setting]] message, or the [[Fetch Setting]] message.
